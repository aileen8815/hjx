...
<div class="panel-body main-content-wrapper site-min-height">
  <form id="outboundBooking-form" action="${base}/store/outbound-booking/${outboundBooking.id!}" method="post" class="form-horizontal"   data-parsley-validate    onsubmit="return getData()">
 ...
 <div class="col-md-6">
      ...
      <select id="customerId" name="customer.id" class="form-control"   onchange="selectedCustomer(this.value)"  data-parsley-required="true">
       ...
       <#list customers as customer>
     	 <option value="${customer.id}" <#if customer.id == outboundBooking.customer.id>selected</#if>>${customer.text!}</option>
	   </#list>
       </select>
      </div>
    </div> 
  ...
  <div class="col-md-6">
    <div class="form-group" >
      ...
      <select id="bookingMethodId" name="bookingMethod.id" class="form-control"  >
       ...
       <#list bookingMethods as bookingMethod>
     	 <option value="${bookingMethod.id}" <#if bookingMethod.id == outboundBooking.bookingMethod.id>selected</#if>>${bookingMethod.name}</option>
	   </#list>
       </select>
      </div>
    </div>
	...
	<div class="col-md-12"  id="showcontactinfo"    <#if !outboundBooking.id?exists> style="display:none" </#if>>
	         <div class="form-group"  >
 ...
 <div class="col-md-12">
    ...
    <div class="row-toolbar">
      ...
      <div class="col-md-12">
        <div style="text-align:right">
 				<label class="sr-only" >选择商品</label>
                <select class="form-control" id="productId" name="productId"   onchange="outboundjs.search(this.value)" style="width:220px;text-align:left;">
  ...
  <#-- JEasyUI DataGrid 显示数据 -->
   ...
   <div  class="col-md-10">
  <table class="easyui-datagrid"  id="outbound-booking-datagrid"   toolbar="#outbound-booking-toolbar"
      data-options="url:'${base}/store/outbound-booking/bookinventory-list?outboundbookingId=${outboundBooking.id}&&customerId=${outboundBooking.customer.id}&&outboundRegisterId=${outboundRegisterId}',method:'get',rownumbers:true,singleSelect:false,selectOnCheck:true,checkOnSelect:false,
    ...
    <thead>
     <tr>
		...
		<th data-options="field:'totalStoreContainer',width:80">库存托数</th>
 ...
 <div class="col-md-12">
    <div class="form-group">
      <div class="col-md-offset-2 col-md-12">
        <button type="submit" class="btn btn-primary"  name="tijiao">保存</button>
      </div>
    </div>
    </div>
    </div>
	...
	var outboundjs = {
	    search: function(){
	      $('#outbound-booking-datagrid').datagrid('load',{
	          ...
	          outboundRegisterId:"${outboundRegisterId}"
	...
	function loadSuccess(data){
		if(data){
			$.each(data.rows, function(index, item){
		      <#list outboundBookingItems as outboundBookingItem>
