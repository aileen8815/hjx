...
<header class="panel-heading">
    <a href="${base}/store/inbound-register/${id}">入库单结算</a>
</header>
<div class="panel-body main-content-wrapper site-min-height">
  <form id="inboundReceiptItem-form" action="${base}/store/inbound-register/${id}/charge" method="post" class="form-horizontal" data-parsley-validate>
    ...
    <div class="form-group">
      ...
      <div class="col-md-offset-2 col-md-2"  id="yanchi"    <#if customer.chargeType.id == 2>style="display:none"</#if>>
    ...
    <div class="form-group">
      <#if "${calculatedResult.feeItem.id!}" == 2 >
	      ...
	      <div class="col-md-2">
	       <input type="hidden" class="form-control" name="feeitem" id="feeitem${calculatedResult_index}"  value="${calculatedResult.feeItem.id!}" data-parsley-required="true" data-parsley-type="number">
	        <input type="text" class="form-control" name="money" id="money${calculatedResult_index}" value="${calculatedResult.amount!}"  readonly="true" data-parsley-required="true" data-parsley-type="number">
	      </div>
	      
	       <label class="col-md-2 control-label">实收${calculatedResult.feeItem.name}：</label>
	      <div class="col-md-2">
	        <input type="text" class="form-control" name="actuallyMoney" id="actuallyMoney${calculatedResult_index}"  value="${calculatedResult.discountedAmount!}" data-parsley-required="true" data-parsley-type="number">
	      </div>
      <#else>
      	  <label class="col-md-2 control-label">应收${calculatedResult.feeItem.name}：</label>
	      <div class="col-md-2">
	       <input type="hidden" class="form-control" name="feeitem" id="feeitem${calculatedResult_index}"  value="${calculatedResult.feeItem.id!}" data-parsley-required="true" data-parsley-type="number">
	        <input type="text" class="form-control" name="money" id="money${calculatedResult_index}" value="0"  readonly="true" data-parsley-required="true" data-parsley-type="number">
	      </div>
     ...
     <#elseif "${calculatedResult.ruleComment!}"?contains("数量")> 
      ...
      $(function(){
		$('#btnCalc${calculatedResult_index}').click(function(){
			...
			{
			var url = "${base}/store/inbound-register/${id}/calculate-item?feeitemId=" + feeItemId + 
