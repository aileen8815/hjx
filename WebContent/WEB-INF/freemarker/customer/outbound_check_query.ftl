...
<header class="panel-heading">
    <a href="${base}/customer/outbound-check-query">出库存信息查询</a>
</header>
<div class="panel-body main-content-wrapper site-min-height">
  <#-- JEasyUI DataGrid 显示数据 -->
  <table class="easyui-datagrid" toolbar="#carrier-toolbar" id="carrier-datagrid" 
      data-options="url:'${base}/customer/outbound-check-query/list',method:'get',rownumbers:true,singleSelect:true,pagination:true,fitColumns:true,collapsible:false">
    <thead>
     <tr>
        ...
        <div class="m-b-sm">
          ...
          <form class="form-inline" operator="form">
            ...
            <div class="form-group">
               <label class="sr-only" for="a">出库时间</label>
                <input type="text" class="form-control Wdate" onClick="WdatePicker()" name="startTime" id="startTime" value="<#if operator.birthday?exists>${operator.birthday?string('yyyy-MM-dd')}</#if>"  placeholder="出库时间">
 
            </div>至
             <div class="form-group">
              <label class="sr-only" for="a">出库时间</label>
                <input type="text" class="form-control Wdate" onClick="WdatePicker()" name="endTime" id="endTime" value="<#if operator.birthday?exists>${operator.birthday?string('yyyy-MM-dd')}</#if>"   placeholder="出库时间">
...
<script type="text/javascript">
  var carrierjs = {
    ...
    var url="${base}/customer/outbound-check-query/"+row.id;
