...
<header class="panel-heading">
    <a href="${base}/ce/simple-rule">计费规则简单维护</a>
</header>
...
<table class="easyui-datagrid" id="rule-datagrid"
    ...
    <thead>
    <tr>
        <th data-options="field:'name', width:200">标准名称</th>
        <th data-options="field:'priority', width:140">优先匹配（从低0到高99）</th>
...
<!--
        <div class='fitem'>
            <label>子项关系</label>
            <input type='radio' id='itemRelation' name='itemRelation' checked='true' value='与'/>与
...
<script type="text/javascript">
    var ce_rule = {
        addEntity: function () {
            ...
            if (row) {
                $('#rule-dlg').dialog('open').dialog('setTitle', '新建收费标准名称');
                $('#rule-fm').form('clear');
        ...
        editEntity: function () {
            ...
            if (row) {
                $('#rule-dlg').dialog('open').dialog('setTitle', '修改收费标准名称');
        ...
        destroyEntity: function () {
            var row = $('#rule-datagrid').datagrid('getSelected');
            if (row) {
                $.messager.confirm('确认', '确实要删除选中的收费标准名称吗？确认将会导致其构成的计费标准、收费标准条件项等数据被级联删除。', function (r) {
                    if (r) {
...
<table id='rule-item-datagrid' class='easyui-datagrid'
    ...
    <tr>
        <th field='optionalItem' width="200px"
            data-options='formatter : function(value, row, index){return row.optionalItem.itemTitle;}'>条件选项
        </th>
        <th field='relation' width="80px"
            data-options='formatter : function(value, row, index){return ce_rule_item.referOperator(value, row);}'>对应关系
        </th>
...
var ce_rule_item = {
    addEntity: function () {
        var row = $('#rule-datagrid').datagrid('getSelected');
        if (row) {
            $('#rule-item-dlg').dialog('open').dialog('setTitle', '新建收费标准条件项');
    ...
    editEntity: function () {
        ...
        if (row) {
            $('#rule-item-dlg').dialog('open').dialog('setTitle', '编辑收费标准条件项');
...
<table id='ce-calculation-item-datagrid' class='easyui-datagrid'
    ...
    <tr>
        ...
        <th field='optionalItem' data-options='formatter : function(value, row, index){if(row.optionalItem) {return row.optionalItem.itemTitle;}}'>计费条件</th>
        <th field='factor'>计费金额</th>
...
<script type="text/javascript">
    var ce_calculation_item = {
        ...
        destroyEntity: function () {
            var row = $('#ce-calculation-item-datagrid').datagrid('getSelected');
            if (row) {
                $.messager.confirm('确认', '确实要删除选中的计费标准吗？确认将导致其下的计算条件也被 级联删除。', function (r) {
                    if (r) {
...
<table id='ce-condition-item-datagrid' class='easyui-datagrid'
    ...
    <thead>
    <tr>
        <th field='optionalItem' data-options='formatter : function(value, row, index){return row.optionalItem.itemTitle;}'>条件选项</th>
        <th field='relation' data-options='formatter : function(value, row, index){return ce_rule_item.referOperator(value, row);}'>对应关系</th>
