  ...
  <div class="col-md-12" id="department-toolbar">
    <div class="row row-toolbar">
      <div class="col-md-7">
        <div class="m-b-sm">
          <div class="btn-group">
            <a href="${base}/settings/department/new" class="btn btn-primary"><i class="fa fa-plus-square"></i> 新建顶级部门</a>
            <button id="department-edit" type="button" class="btn btn-primary" onclick="departmentjs.addSub();"><i class="fa fa-plus"></i> 新建下级部门</button>
...
<script type="text/javascript">
  var departmentjs = {
    addSub: function(){
      ...
      if (row){
        location.href = "${base}/settings/department/new?parentId=" + row.id;
      } else {
        alert('请选择上级部门！');
      }
    },
    ...
    edit: function(){
      var row = $('#department-treegrid').treegrid('getSelected');  
      if (row){
        location.href = "${base}/settings/department/" + row.id + "/edit";
      } else {
        alert('请选择要编辑的数据！');
      }
    },
    ...
    remove: function(){
      ...
      if (row){
        if(confirm('确实要删除选中数据吗？')){
          location.href = "${base}/settings/department/" + row.id + "/delete";
        }
      }else{
        alert('请选择要删除的数据！');
      }
    ...
    dnd: function(targetRow,sourceRow,point){
      var url = "${base}/settings/department/" + sourceRow.id + "/organise?parentId=" + targetRow.id + "&point=" + point;
